                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Dossiers</h1>
                                ...
                                $today = date('Y-m-d');
                                echo $today;
                                $allodoc = $database->query("SELECT patient.pname, dossier.did, dossier.pid, dossier.date_c, dossier.date_maj FROM patient JOIN dossier ON patient.pid = dossier.pid WHERE dossier.pid='$userid'");
                                ?>
                                </p>
                    </div>
                    <!-- Content Row -->
                        ...
                        <div class="col-xl-12 col-lg-12"  width="100%">
                            <div class="card shadow mb-12"  width="100%">
                                ...
                                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between"  width="100%">
                                    <h6 class="m-0 font-weight-bold text-primary">Dossiers (<?php echo $allodoc->num_rows  ?>)</h6>
                                </div>
                                <?php
                                    if($_POST && isset($_POST["search"])){
                                        $keyword = $_POST["search"];
                                        $sqlmain = "SELECT patient.pname FROM patient JOIN dossier ON patient.pid = dossier.pid WHERE patient.pid='$keyword' OR patient.pid='$keyword' OR patient.pid LIKE '$keyword%' OR patient.pid LIKE '%$keyword' OR patient.pid LIKE '%$keyword%'";
                                    }else{
                                        $sqlmain = "SELECT patient.pname, dossier.did, dossier.pid, dossier.date_c, dossier.date_maj FROM patient JOIN dossier ON patient.pid = dossier.pid WHERE dossier.pid='$userid'";
                                    }                
                                    ?>
                                <div class="card-body" >
                                <div class="table-responsive">
                                <table class="table table-bordered" id="dataTable1" width="100%" cellspacing="0">
                                    ...
                                    <th>Date de creation du dosseir</th>
                                    <th>Date de la mise a jour du dossier</th>
                                    <th>Actions</th>
                                    </thead>
                                    <tbody>
                                    <?php
                                        ...
                                        for ($x = 0; $x < $result->num_rows; $x++) {
                                            $row = $result->fetch_assoc();
                                            $did = isset($row["did"]) ? $row["did"] : null;
                                            $pid = isset($row["pid"]) ? $row["pid"] : null;
                                            $pname = isset($row["pname"]) ? $row["pname"] : null;
                                            $date_c = isset($row["date_c"]) ? $row["date_c"] : null;
                                            $date_maj = isset($row["date_maj"]) ? $row["date_maj"] : null;
                                            echo '<tr>
                                                    <td>' . $did . '</td>
                                                    <td>' . $pname . '</td>
                                                    <td>' . $date_c . '</td>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="#" class="non-style-link view-doc" data-did="' . $did . '" data-pid="' . $pid . '">
