                ...
                <div class="container-fluid">
                    ...
                    <td colspan="4" style="padding-top:0px;width: 100%;" >
                        ...
                        <form action="" method="post">
                            <input type="date" name="sheduledate" id="date" class="form-control" style="margin: 0;width: 95%;">
                        </td>
                    <td width="12%">
                        <input type="submit"  name="filter" value=" Filter" class="btn btn-primary"  style="padding: 15px; margin :0;width:95%">
                        </form>
                    </td>
                    </tr>
                            </table>
                        ...
                        <?php
                        $sqlmain= "SELECT prescription.prid, patient.pname, doctor.docname, medoc.nom AS nom_m, traitement.nom AS nom_t, prescription.date_prescr, prescription.date_fins, prescription.instruction
                        FROM prescription
                        ...
                        ";
                        if($_POST){
                            if(!empty($_POST["precrition"])){
                                $prescription=$_POST["prescription"];
                                $sqlmain.=" and prescription.date_prescr='$prescription' ";
                            };
                        }
                        $sqlmain.="order by prescription.date_prescr  asc";
                        $result= $database->query($sqlmain);
                                ?>
                            <div class="card shadow mb-12"  width="100%">
                               ...
                               if($result->num_rows==0){
                                ...
                                echo "<div style='display: flex; flex-wrap: wrap; justify-content: space-between;'>"; // Ajoutez les styles flexbox ici
                                for ($x = 0; $x < ($result->num_rows); $x++) {
                                    for ($q = 0; $q < 3; $q++) {
                                        $row = $result->fetch_assoc();
                                        if (!isset($row)) {
                                        ...
                                        $docname = $row["docname"];
                                        $medoc = $row["nom_m"];
                                        $traitement = $row["nom_t"];
                                        $date_prescr = $row["date_prescr"];
                                        $date_fins = $row["date_fins"];
                                        $instruction = $row["instruction"];
                            
                                        // Utilisez une combinaison de $x et $q pour générer un identifiant unique
                                        $uniqueId = 'printableArea' . $x . '_' . $q;
                            
                                        echo '
                                        <div style="flex: 0 0 calc(33.33% - 20px); margin: 10px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2); transition: 0.3s; border-radius: 5px; padding: 20px; box-sizing: border-box; color: #000; display: flex; flex-direction: column; justify-content: space-between;">
                                            <div id="' . $uniqueId . '">
                                                <div style="width:100%;">
                                                    <div class="h3-search">
                                                    <strong>Prescription Date: </strong>' . substr($date_prescr, 0, 30) . '<br>
                                                        <strong>Reference Number:</strong> PR-000- ' . $prid . '
                                                    </div>
                                                    <div class="h1-search">
                                                    <strong>Nom du (de la) Patient(e) :</strong> ' . substr($pname, 0, 21) . '
                                                    </div>
                                                    <div class="h1-search">
                                                    <strong> Le(La) Docteur(e) en Charge : </strong>' . substr($docname, 0, 30) . '
                                                    </div>
                                                    <div class="h3-search">
                                                    <strong>  Nom Du Medicament: </strong>' . $medoc . '<br>
                                                    </div>
                                                    <div class="h3-search">
                                                    <strong>  Nom Du Traitement:</strong> ' . $traitement . '<br>
                                                    </div>
                                                    <div class="h3-search">
                                                    <strong> Instructions : </strong>' . $instruction . '<br>
                                                    </div>
                                                </div>
                                            </div>
                                            <div>
                                                <button onclick="printContent(\'' . $uniqueId . '\')" class="btn btn-primary" style="padding-top:11px;padding-bottom:11px;margin: 5px;width: 100%;">
                                                    <i class="fas fa-print"></i> <font class="tn-in-text">Imprimer</font>
                                                </button>
                                            </div>
                                        </div>
                                        ';
                                ...
                                echo "</div>"; // Fermez le conteneur flexbox ici
                            }
                                ...
                                <script>
                                    ...
                                    // Récupérer l'élément à imprimer et son contenu
                                    ...
                                    iframe.contentDocument.write('<html><body>' + printContent + '</body></html>');
                                ...
                                <script>
                                    function confirmCancellation(appoid, title, docname) {
                                    Swal.fire({
                                        title: 'Êtes-vous sûr ?',
                                        text: `Voulez-vous vraiment annuler la réservation pour la session "${title}" avec le docteur ${docname} ?`,
                                        icon: 'warning',
                                ...
                                if($_GET){
                                    ...
                                    if($action=='booking-added'){
                                        ...
                                        <script>
                                        Swal.fire({
                                            title: "Resrvation fait avec succès",
                                            text: "votre numéro de rendez-vous est : '.$id.'",
                                            icon: "success",
