...
if (isset($_GET['id'])) {
    ...
    $sqlmain = "SELECT schedule.scheduleid, schedule.title, doctor.docname, doctor.docid, schedule.scheduledate, schedule.scheduletime, schedule.nop
                FROM schedule 
    ...
    if ($result->num_rows > 0) {
        ...
        $docname = $row["docname"];
        $docid = $row["docid"]; // Voici le docid
        ...
        $response = [
            ...
            'data' => [
                'docname' => $docname,
                'docid' => $docid, // Ajoutez le docid à la réponse
